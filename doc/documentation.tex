...
This is implemented by reading in a text file containing the 3-dimensional matter power spectrum $P(k)$ in the \textit{synchronous gauge}, (with units Mpc/$h)^3$, and the wavenumber $k=\sqrt{k_x^2+k_y^2+k_z^2}$ (with units $h$/Mpc). Here $h$ is defined such that $H_0 = 100 h$ km/s/Mpc. It is assumed the text file is the same format as the matter power spectrum output from CAMB\footnote{\url{https://camb.info}} or CLASS\footnote{\url{https://class-code.net}}, i.e. a text file with two columns $k, P(k)$ and no $k=0$ value (this is added by the code). 

\texttt{FLRWSolver}\ then generates a Gaussian random field and scales it to the given power spectrum. This field is centred around zero and represents the initial (dimensionless) density perturbation to the EdS background in synchronous gauge, $\delta_s$. We choose to first calculate $\delta_s$ because CAMB/CLASS by default output $P(k)$ in the synchronous gauge, and there is some translation required to get $P(k)$ directly in longitudinal gauge. After generating $\delta_s$ we calculate the gauge-invariant Bardeen potential using the Poisson equation, namely,
\begin{equation}
	\nabla^2 \Phi = \frac{3\mathcal{H}^2}{2}\delta_s,
\end{equation}
and the metric perturbation in longitudinal gauge is $\phi=\Phi$. We then use equations \eqref{eqs:linear_solnsg0} in Fourier space to find the corresponding density and velocity perturbations in the longitudinal gauge (to linear order), which are then inverse transformed back to give the real-space perturbations.
